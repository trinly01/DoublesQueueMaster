...
const OUT_PATH = path.join(
  ...
  'test',
  'fixtures',
  'ideal-composite-score-output.txt',
);
...
const results = [];
for (let alpha = 0; alpha <= 2.5; alpha += 0.1) {
  for (let beta = 0; beta <= 8; beta += 0.2) {
    const scoreFn = (p) => {
      const winRate = p.matchesPlayed ? p.wins / p.matchesPlayed : 0;
      return p.rating + alpha * 1000 * (winRate - 0.5) - beta * p.matchesPlayed;
    };
...
results.sort((a, b) => a.inversions - b.inversions || a.eddieAbove - b.eddieAbove);

const lines = [];
lines.push('=== COMPOSITE SCORE SEARCH ===');
lines.push('Formula: score = rating + alpha*1000*(winRate - 0.5) - beta*games');
lines.push('');
lines.push('Top 10 combos by ranking match');
lines.push(
  'Alpha  Beta  Inv  EddieAbove  Ranks (ideal players 1-9)',
);
...
lines.push(`score = rating + ${best.alpha.toFixed(1)}*1000*(winRate - 0.5) - ${best.beta.toFixed(1)}*games`);
...
console.log(`Composite search complete. Results written to ${OUT_PATH}`);
console.log(`Best: alpha=${best.alpha.toFixed(1)}, beta=${best.beta.toFixed(1)}, inversions=${best.inversions}, eddieAbove=${best.eddieAbove}`);
