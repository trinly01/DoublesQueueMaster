...
import { scorePredictions, bootstrapCI, walkForwardCV } from './lib/evaluate.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const OUT_PATH = path.join(__dirname, '..', 'test', 'fixtures', 'exp-05-optimization-output.txt');
...
function iteratedReplay(matches, params, maxPasses, options = {}) {
  const { perPassShrinkage = false, teamRatingMode = 'arithmetic' } = options;
  ...
  for (let pass = 0; pass < maxPasses; pass++) {
    ...
    if (perPassShrinkage && params.shrinkagePriorWeight > 0) {
      for (const p of Object.values(result.players)) {
        ...
        if (n > 0) {
          p.rating = p.initialRating + (p.rating - p.initialRating) * (n / (n + params.shrinkagePriorWeight));
        }
      }
...
function evalConfig(matches, params, passes, options = {}) {
  ...
  const ratings = Object.values(r.players).filter(p => p.matchesPlayed > 0).map(p => p.rating);
...
for (const k of [20, 24, 28, 32, 36, 40, 44, 48]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: k, autocorrScale: 1000, shrinkagePriorWeight: 12 };
  const r = evalConfig(matches, params, 3);
  const marker = r.logLoss < bestKLL ? ' ***' : '';
  log('  K=' + String(k).padEnd(4) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' max=' + r.maxR + marker);
  if (r.logLoss < bestKLL) { bestKLL = r.logLoss; bestK = k; }
}
...
for (const passes of [1, 2, 3, 4, 5, 7, 10]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: 1000, shrinkagePriorWeight: 12 };
  const r = evalConfig(matches, params, passes);
  const marker = r.logLoss < bestPassLL ? ' ***' : '';
  log('  passes=' + String(passes).padEnd(4) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' max=' + r.maxR + marker);
  if (r.logLoss < bestPassLL) { bestPassLL = r.logLoss; bestPasses = passes; }
}
...
for (const c of [0, 4, 8, 12, 16, 20, 24, 30]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: 1000, shrinkagePriorWeight: c };
  const r = evalConfig(matches, params, bestPasses);
  const marker = r.logLoss < bestCLL ? ' ***' : '';
  log('  C=' + String(c).padEnd(4) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' max=' + r.maxR + marker);
  if (r.logLoss < bestCLL) { bestCLL = r.logLoss; bestC = c; }
}
log('  Best C=' + bestC + ' (logLoss=' + bestCLL.toFixed(4) + ')');
log('');

// Step 4: Partner gap sweep (with best K, passes, C)
log('--- Step 4: Partner Gap Factor (K=' + bestK + ', passes=' + bestPasses + ', C=' + bestC + ') ---');
let bestGap = 0.5;
let bestGapLL = 999;
for (const gap of [0, 0.25, 0.5, 0.75, 1.0, 1.5]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: 1000, shrinkagePriorWeight: bestC, partnerGapFactor: gap };
  const r = evalConfig(matches, params, bestPasses);
  const marker = r.logLoss < bestGapLL ? ' ***' : '';
  log('  gap=' + String(gap).padEnd(5) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' max=' + r.maxR + marker);
  if (r.logLoss < bestGapLL) { bestGapLL = r.logLoss; bestGap = gap; }
}
...
for (const auto of [0, 250, 500, 750, 1000, 1500, 2200]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: auto, shrinkagePriorWeight: bestC, partnerGapFactor: bestGap };
  const r = evalConfig(matches, params, bestPasses);
  const marker = r.logLoss < bestAutoLL ? ' ***' : '';
  log('  auto=' + String(auto).padEnd(5) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' max=' + r.maxR + marker);
  if (r.logLoss < bestAutoLL) { bestAutoLL = r.logLoss; bestAuto = auto; }
}
...
for (const blend of [0, 0.25, 0.5, 0.75, 1.0]) {
  const params = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: bestAuto, shrinkagePriorWeight: bestC, partnerGapFactor: bestGap, lossUnderdogBlend: blend };
  const r = evalConfig(matches, params, bestPasses);
  const marker = r.logLoss < bestBlendLL ? ' ***' : '';
  log('  blend=' + String(blend).padEnd(5) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + marker);
  if (r.logLoss < bestBlendLL) { bestBlendLL = r.logLoss; bestBlend = blend; }
}
...
log('Baseline: K=' + bestK + ' passes=' + bestPasses + ' C=' + bestC + ' gap=' + bestGap + ' auto=' + bestAuto + ' blend=' + bestBlend);
log('');

const baselineParams = { ...DEFAULT_PARAMS, kDoubles: bestK, autocorrScale: bestAuto, shrinkagePriorWeight: bestC, partnerGapFactor: bestGap, lossUnderdogBlend: bestBlend };
const baselineR = evalConfig(matches, baselineParams, bestPasses);
log('Baseline: logLoss=' + baselineR.logLoss.toFixed(4) + ' spread=' + baselineR.spread);
log('');

// Variant A: Per-pass shrinkage
log('--- Variant A: Per-pass Shrinkage ---');
const rA = evalConfig(matches, baselineParams, bestPasses, { perPassShrinkage: true });
log('  logLoss=' + rA.logLoss.toFixed(4) + ' spread=' + rA.spread + ' delta=' + (rA.logLoss - baselineR.logLoss).toFixed(4));
log('');

// Variant B: No shrinkage at all
log('--- Variant B: No Shrinkage ---');
const rB = evalConfig(matches, { ...baselineParams, shrinkagePriorWeight: 0 }, bestPasses);
log('  logLoss=' + rB.logLoss.toFixed(4) + ' spread=' + rB.spread + ' delta=' + (rB.logLoss - baselineR.logLoss).toFixed(4));
log('');
...
for (const passes of [bestPasses + 1, bestPasses + 2, bestPasses + 3]) {
  ...
  log('  passes=' + String(passes).padEnd(4) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' delta=' + (r.logLoss - baselineR.logLoss).toFixed(4));
}
log('');
...
for (const c of [bestC - 4, bestC, bestC + 4, bestC + 8]) {
  ...
  const r = evalConfig(matches, params, bestPasses);
  log('  C=' + String(c).padEnd(4) + 'logLoss=' + r.logLoss.toFixed(4) + ' spread=' + r.spread + ' delta=' + (r.logLoss - baselineR.logLoss).toFixed(4));
}
log('');
...
log('Club A: ' + clubA.length + ' matches, Club B: ' + clubB.length + ' matches');
log('');

// Compare current config vs optimized
const currentParams = { ...DEFAULT_PARAMS, kDoubles: 64, marginWeight: 0.15, autocorrScale: 2200, shrinkagePriorWeight: 12, partnerGapFactor: 0.5, movMode: 'log' };
...
log('  ClubA=' + curA.logLoss.toFixed(4) + ' ClubB=' + curB.logLoss.toFixed(4) + ' avg=' + ((curA.logLoss+curB.logLoss)/2).toFixed(4));
log('');
...
log('  ClubA=' + optA.logLoss.toFixed(4) + ' ClubB=' + optB.logLoss.toFixed(4) + ' avg=' + ((optA.logLoss+optB.logLoss)/2).toFixed(4));
log('');
...
log('Improvement: ' + ((0.6270 - baselineR.logLoss) / 0.6270 * 100).toFixed(1) + '%');
log('Spread: ' + baselineR.spread + ' (current: ~881)');
log('Max rating: ' + baselineR.maxR);
log('');
log('Cross-club: ClubA=' + optA.logLoss.toFixed(4) + ' ClubB=' + optB.logLoss.toFixed(4));
