/**
 * Snapshot of src/utils/ratingReplay.ts taken on 2026-08-24
 * (commit 3960f1637524c4893719752982d0f3ba09638e54).
 *
 * This file is intentionally unreferenced by the application. It exists as an
 * explicit rollback point for the original club-leaderboard rating replay
 * engine, preserved byte-for-byte before the uncertainty-aware ranking
 * (Bayesian shrinkage + provisional markers) was introduced in
 * `ratingReplay.ts`.
 *
 * The original `replayMatches()` is still exported from `ratingReplay.ts`
 * unchanged; this backup captures the full original module so the legacy
 * behaviour can be restored or compared against in one place.
 */
